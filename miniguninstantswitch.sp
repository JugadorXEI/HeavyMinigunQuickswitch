	...
	{	
		...
		if (iClass == TFClass_Heavy)
		{
			...
			if (IsValidEdict(iActiveWeapon))
			{
				...
				{
					int iWeaponState = GetEntProp(iActiveWeapon, Prop_Send, "m_iWeaponState");
					
					if (iWeaponState > 0 && iWeaponToSwitchTo != iActiveWeapon)
					{
						//PrintToChat(iClient, "bep5");
						TF2_RemoveCondition(iClient, TFCond_Slowed);
						SetEntProp(iActiveWeapon, Prop_Send, "m_iWeaponState", 0);
